...
export function FieldRadioPlaygroundDemo() {
  const [label, setLabel] = React.useState("Compute environment");
  const [description, setDescription] = React.useState(
    "Select the compute environment for your cluster.",
  );
  const [error, setError] = React.useState("");
  const [asCard, setAsCard] = React.useState(false);
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-fd-border md:flex-row">
      ...
      <div className="w-full shrink-0 space-y-5 border-t border-fd-border bg-fd-muted/20 p-5 md:w-72 md:border-l md:border-t-0">
        <Toggle checked={asCard} onChange={setAsCard} label="As card" />
        <Toggle
          checked={required}
