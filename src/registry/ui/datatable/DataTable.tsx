...
import { getRecordId, getColumnKey, computeOrderedColumns, computePinOffsets, useMergedRef } from "./utils";
...
>(
  props: BaseDataTableProps<T> & { columns: Cols } & (Cols[number]["sortable"] extends true
      ? RequiredSortProps<T>
      : SortProps<T>)
) {
  const {
  ...
  const handleToggleSelect = React.useCallback(
    (record: T, rowIndex: number, shiftKey: boolean) => {
      ...
      if (shiftKey && lastSelectionIndex !== null) {
        const [start, end] =
          lastSelectionIndex < rowIndex ? [lastSelectionIndex, rowIndex] : [rowIndex, lastSelectionIndex];
        ...
        if (willSelect) {
          ...
          const rangeIds = new Set(rangeRecords.map((r) => getRecordId(r, idAccessor)));
          onSelectedRecordsChange((selectedRecords ?? []).filter((r) => !rangeIds.has(getRecordId(r, idAccessor))));
        }
      } else if (selectedIds.has(id)) {
        onSelectedRecordsChange((selectedRecords ?? []).filter((r) => getRecordId(r, idAccessor) !== id));
      } else {
        onSelectedRecordsChange([...(selectedRecords ?? []), record]);
      }
      setLastSelectionIndex(rowIndex);
    },
    [onSelectedRecordsChange, selectedRecords, selectedIds, idAccessor, data, lastSelectionIndex]
  );
  ...
  const handleToggleAll = React.useCallback(() => {
    ...
    if (allChecked) {
      ...
      onSelectedRecordsChange((selectedRecords ?? []).filter((r) => !idsOnPage.has(getRecordId(r, idAccessor))));
    } else {
      const merged = [...(selectedRecords ?? [])];
      const existing = new Set(merged.map((r) => getRecordId(r, idAccessor)));
      for (const r of data) {
  ...
  const [internalExpanded, setInternalExpanded] = React.useState<Set<React.Key>>(new Set());
  const isExpandControlled = expandedRecordIds !== undefined;
  const expandedSet = React.useMemo(
    () => new Set(isExpandControlled ? expandedRecordIds : Array.from(internalExpanded)),
  ...
  const toggleExpand = React.useCallback(
    ...
    [expandedSet, singleExpand, isExpandControlled, onExpandedRecordIdsChange, idAccessor]
  );
  ...
  const handleSort = React.useCallback(
    (col: DataTableColumn<T>) => {
      ...
      const nextDirection: SortDirection = isCurrent && sortStatus?.direction === "asc" ? "desc" : "asc";
      onSortStatusChange({ columnAccessor: col.accessor, direction: nextDirection });
    },
  ...
  const orderedColumns = React.useMemo(() => computeOrderedColumns(columns), [columns]);
  const leadingGutter = (selectable ? SELECTION_COL_WIDTH : 0) + (expandable ? EXPAND_COL_WIDTH : 0);

  const { leftOffsets, rightOffsets, leftPinnedCols, rightPinnedCols } = React.useMemo(
  ...
  const pinnedCellStyle = React.useCallback(
    (col: DataTableColumn<T>): React.CSSProperties => {
      ...
      if (col.pinned === "left") return { position: "sticky", left: leftOffsets[key] };
      if (col.pinned === "right") return { position: "sticky", right: rightOffsets[key] };
      return {};
    },
  ...
  const pinnedCellClassName = React.useCallback(
    (col: DataTableColumn<T>, zIndexClass: string, bgClassName: string = "bg-background") => {
      const isLastLeft = leftPinnedCols.length > 0 && leftPinnedCols[leftPinnedCols.length - 1] === col;
      const isFirstRight = rightPinnedCols.length > 0 && rightPinnedCols[0] === col;
      return cn(
        col.pinned && "sticky",
        col.pinned && bgClassName,
        col.pinned && zIndexClass,
        col.pinned === "left" && isLastLeft && "shadow-[2px_0_4px_-2px_rgba(0,0,0,0.15)]",
        col.pinned === "right" && isFirstRight && "shadow-[-2px_0_4px_-2px_rgba(0,0,0,0.15)]"
      );
    },
  ...
  const totalCols = orderedColumns.length + (selectable ? 1 : 0) + (expandable ? 1 : 0);

  /* --------------------------------- Render --------------------------------- */

  return (
    <div className={cn("rounded-md", variant === "bordered" && "border", className)}>
      <div className="relative">
        ...
        >
          <Table
            ...
            style={{ tableLayout: columns.some((c) => c.width) ? "fixed" : "auto" }}
          >
            <DataTableHeader<T>
              ref={headerRef}
            ...
            <TableBody ref={bodyRef}>
              {data.length === 0 ? (
                <DataTableEmptyState text={fetching ? "" : noRecordsText} colSpan={totalCols} />
              ) : (
                data.map((record, rowIndex) => {
                  const rowId = getRecordId(record, idAccessor);
                  const isSelected = selectedIds.has(rowId);
                  const isExpanded = expandedSet.has(rowId);
                  const canExpand = expandable && (rowExpandable ? rowExpandable(record, rowIndex) : true);
                  ...
                  const resolvedRowStyle = typeof rowStyle === "function" ? rowStyle(record, rowIndex) : rowStyle;
      ...
      {pagination && (
        <DataTablePagination pagination={pagination} variant={variant} sticky={sticky} footerRef={footerRef} fetching={fetching} />
      )}
    </div>
  );
}
